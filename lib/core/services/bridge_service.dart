...
const _bindingName = '__shimxBridge';
const _bootstrapAssetPath = 'assets/inject/bridge_bootstrap.js';
const _devBootstrapPath =
    r'F:\Programming_projects\FlutterProject\shimx\assets\inject\bridge_bootstrap.js';
...
/// page 里暴露 window.shimx(path, payload)，调用经 binding 路由到 dart handler。
class BridgeService {
  ...
  Future<void> _replyJs(String id, Map<String, dynamic> envelope) {
    return _cdp.evaluate(
      'window.__shimxResolve(${jsonEncode(id)}, ${jsonEncode(envelope)})',
    );
  }

  /// 从 dart 主动向 JS 推一条事件。JS 侧用 window.__shimxEvent(path, listener) 订阅。
  /// 失败静默(可能注入未就绪),不抛出。
  void dispatchEvent(String path, Map<String, dynamic> payload) {
    () async {
      try {
        await _cdp.evaluate(
          'window.__shimxDispatch && window.__shimxDispatch(${jsonEncode(path)}, ${jsonEncode(payload)})',
        );
      } catch (_) {}
    }();
  }
